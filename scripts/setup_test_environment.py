...
result = subprocess.run([sys.executable, "create_test_accounts.py"], capture_output=True, text=True)
...
if not admin_user:
    print(
        f"       ❌ Admin-User wurde nicht angelegt. Bitte prüfen, ob das Script im richtigen Virtualenv lief (sys.executable = {sys.executable})"
    )
    sys.exit(1)
else:
    print(f"       ✓ Admin-User vorhanden ({admin_user.username} / admin123)")

# Step 3: Erstelle Test-Einträge und Coupons
print("\n[3/5] Erstelle Test-Daten...")
from app import app, db  # noqa: E402
from spo.models import BonusProgram, Coupon, Shop, ShopProgramRate  # noqa: E402
...
payback_shop = (
    Shop.query.join(ShopProgramRate).filter(ShopProgramRate.program_id == payback.id).first()
)
...
old_coupons = Coupon.query.filter(Coupon.status == "active").all()
...
print(
    f"       Shops mit Coupons: {', '.join([s.name for s in test_shops[:len(set([c['shop_idx'] for c in coupons_to_create]))]])}"
)
