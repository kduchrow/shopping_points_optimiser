    ...
    def evaluate():
        ...
        if mode == 'shopping':
            ...
            for rate in rates:
                ...
                best_discount = max((c.value for c in program_coupons if c.coupon_type == 'discount'), default=0)
            ...
            results.sort(key=lambda r: r['coupon_info']['euros'] if r.get('coupon_info') else r['euros'], reverse=True)
        ...
        for rate in rates:
            program = BonusProgram.query.get(rate.program_id)
            results.append({'program': program.name, 'note': 'Vertragsabschluss - siehe Admin für genaue Angaben'})
