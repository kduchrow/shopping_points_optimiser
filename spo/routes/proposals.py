...
def register_proposals(app):
    ...
    def proposals():
        ...
        metadata_proposals = ShopMetadataProposal.query.filter_by(status='PENDING').order_by(ShopMetadataProposal.created_at.desc()).all()
        metadata_data = []
        ...
        merge_proposals = ShopMergeProposal.query.filter_by(status='PENDING').order_by(ShopMergeProposal.created_at.desc()).all()
        merge_data = []
        ...
        for merge in merge_proposals:
            ...
            merge_data.append(
                {'proposal': merge, 'variant_a': variant_a, 'variant_b': variant_b, 'submitter': submitter}
            )

        return render_template(
            'proposals.html', proposals_data=proposals_data, metadata_data=metadata_data, merge_data=merge_data
        )
        ...
        if request.method == 'POST':
            ...
            if proposal_type in ['rate_change', 'shop_add', 'program_add', 'coupon_add']:
                ...
                if proposal_type == 'rate_change':
                    ...
                    if coupon_valid_to:
                        proposal.proposed_coupon_valid_to = datetime.strptime(coupon_valid_to, '%Y-%m-%d')
