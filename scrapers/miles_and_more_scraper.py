        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if not partner_links:
                    partner_links = page.locator('a.partner-tile, a[href*="/program/partners/"]').all()
                if not partner_links:
                    print("   [!] Keine Partner-Links gefunden. Eventuell Cookie-Banner blockiert oder Layout geaendert.")
                partner_urls = []
                    ...
                    try:
                        ...
                        if href and '/partners/' in href:
                            ...
                            if shop_name and not shop_name.startswith('/'):
                                partner_urls.append({
                                    'name': shop_name,
                                    'url': href if href.startswith('http') else f"https://www.miles-and-more.com{href}"
                                })
                    except Exception:
                        continue
                ...
                for i, partner in enumerate(partner_urls, 1):
                    try:
                        print(f"   [{i}/{len(partner_urls)}] Scraping {partner['name']}...", end='', flush=True)
                ...
                try:
                    ...
                    print("   [+] Saved debug artifacts: miles_more_error.png / miles_more_error.html")
                except Exception:
                    pass
                browser.close()
