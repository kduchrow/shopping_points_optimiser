...
class PaybackScraperJS(BaseScraper):
    ...
    def _extract_numeric(self, text):
        ...
        m = re.search(r"(\d+[\.,]?\d*)\s*(?:°P|Punkt|Punkte)\s*(?:pro|je)\s*(\d+[\.,]?\d*)\s*(?:€|EUR|Euro)", text, re.I)
        if m:
            pts = float(m.group(1).replace(',', '.'))
            per = float(m.group(2).replace(',', '.'))
            result['points_per_eur'] = pts / per if per != 0 else 0.0
            print(f"[DEBUG] Extracted points_per_eur from '{text}': {pts} / {per} = {result['points_per_eur']}")
            return result
        ...
        if 'abschluss' not in text.lower() and 'vertrag' not in text.lower():
            m = re.search(r"(\d+[\.,]?\d*)\s*(?:°P|Punkte|Punkt)(?:\s*je\s*(?:€|EUR|Euro))?", text, re.I)
            if m:
                pts = float(m.group(1).replace(',', '.'))
            ...
            with sync_playwright() as p:
                ...
                while button_clicks < max_clicks:
                    ...
                    try:
                        ...
                        if added <= 0:
                            print(f"No new tiles loaded after click (was {current_count}, now {new_count}).")
                ...
                for i, tile in enumerate(tiles):
                    try:
                        ...
                        per_completion = None
                        m = re.search(r"(\d+[\.,]?\d*)\s*°?P\s*(?:pro|pro Abschluss)", incentive, re.I)
                        if m:
